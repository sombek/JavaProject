...
public class Account implements Serializable {
	...
	public Account(String name, int days, boolean breakfast, int roomType, int id) {
		this.name = name;
		this.days = days;
	...
	public boolean equals(Object other) {
		return this.id == ((Account) other).getId();
	}
	...
	public static Account getAccount() {
		...
		try {
			...
			System.out.println();
			return new Account(name, days, breakfast, roomType, id);
		} catch (Exception e) {
	...
	public static Account findAccount(){
		...
		int id = input().nextInt();
		return new Account("", 0, true, 1, id);
	}
